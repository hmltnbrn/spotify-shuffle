...
class TrackList extends Component<Props> {
  ...
  playTrack = (track, index) => {
    this.props.playTrack(track, index, this.props.playlistIndex);
  }

  render () {
    const { tracks, trackIndex } = this.props;
    const trackView = tracks.map((item, index) => {
      ...
      return (
        <TouchableHighlight key={index} onPress={() => this.playTrack(item.track, index)} underlayColor="#fafafa">
    ...
    return (
      <View style={styles.container}>
        <ScrollView>
          <TouchableWithoutFeedback>
            <View style={styles.scrollContainer}>{trackView}</View>
          </TouchableWithoutFeedback>
        </ScrollView>
      </View>
    );
